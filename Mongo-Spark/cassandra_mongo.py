...
def TrainAndComputeRecommendation(sc, data, 
...
#   model.save(sc, "target/tmp/PysparkCollaborativeFiltering")
    ...
    user_items = datafile.map(
        lambda l: (int(l[0]),int(l[1]))).groupByKey()
    ...
    user_item_pairs = datafile.map(
        lambda l: ((int(l[0]),int(l[1])),float(l[2]))).reduceByKey( # in case of multiple purchases
        lambda x,y:x+y).map(
...
if __name__ == "__main__":
    ...
    data = sc.cassandraTable("mykeyspace", "transactions",row_format=1).collect()
    datafile = sc.parallelize(data).map(list)
    #===========================================================================
    ...
    #=======================================================================
    save_rdd_to_mango.activate()
    save_rdd_to_mango.clear_collections()

    save_rdd_to_mango.save(recomm_per_user,'test_database.recomm_per_user')
    save_rdd_to_mango.save(recomm_per_item,'test_database.recomm_per_item')
    
    elapsed = (time.time() - t0) 
    print ("\nIt took %.2fsec to complete the whole process" % elapsed) 
    #=======================================================================
